...
;
SegaPCM		equ $F8000+$080000
SegaPCM_End 	equ SegaPCM+$5E2F
SEGA_Size	equ	(SegaPCM_End-SegaPCM)&$FFFF; The size of the SEGA sound
SEGA_Location	equ	SegaPCM&$7FFF|$8000; The location within the bank of the SEGA sound, 8000h-based
SEGA_Pitch	equ	$0D		; The pitch of the SEGA sound
SEGA_Bank	equ	((SegaPCM&$FF8000)/$8000)>>1; The location of the bank where the SEGA sound is in the main genesis ROM (multiply it by $10000 and add the next equate multiplied by $1000 to it to get the actual bank's location in ROM)
SEGA_Bank_Minor	equ	(((SegaPCM&$FF8000)/$8000)&1); The location of the bank where the SEGA sound is in the main genesis ROM (multiply it by $1000 and add the previous equate multiplied by $10000 to it to get the actual bank's location in ROM)
