...
using SonicRetro.SonLVL.API.S3K;
using System;
...
namespace SonicRetro.SonLVL.GUI
{
	public partial class MainForm : Form
	{
		...
		/*	if (File.Exists("SonLVL Updater.exe"))
			...
			dirlist.SmallImageList = imgList;
			swaplist.SmallImageList = imgList;
			sszlist.SmallImageList = imgList;
		}
		...
		private void ScrollBar_ValueChanged(object sender, EventArgs e) {
			...
			switch (CurrentTab) {
				case Tab.Objects:
					...
					swapPanel.HScrollValue = Math.Min(objectPanel.HScrollValue, swapPanel.HScrollMaximum);
					swapPanel.VScrollValue = Math.Min(objectPanel.VScrollValue, swapPanel.VScrollMaximum);
					dirPanel.HScrollValue = Math.Min(objectPanel.HScrollValue, dirPanel.HScrollMaximum);
					dirPanel.VScrollValue = Math.Min(objectPanel.VScrollValue, dirPanel.VScrollMaximum);
					break;
				case Tab.Foreground:
					...
					swapPanel.HScrollValue = Math.Min(foregroundPanel.HScrollValue, swapPanel.HScrollMaximum);
					swapPanel.VScrollValue = Math.Min(foregroundPanel.VScrollValue, swapPanel.VScrollMaximum);
					dirPanel.HScrollValue = Math.Min(foregroundPanel.HScrollValue, dirPanel.HScrollMaximum);
					dirPanel.VScrollValue = Math.Min(foregroundPanel.VScrollValue, dirPanel.VScrollMaximum);
					break;
				case Tab.Directions:
					swapPanel.HScrollValue = Math.Min(dirPanel.HScrollValue, swapPanel.HScrollMaximum);
					swapPanel.VScrollValue = Math.Min(dirPanel.VScrollValue, swapPanel.VScrollMaximum);
					objectPanel.HScrollValue = Math.Min(dirPanel.HScrollValue, objectPanel.HScrollMaximum);
					objectPanel.VScrollValue = Math.Min(dirPanel.VScrollValue, objectPanel.VScrollMaximum);
					foregroundPanel.HScrollValue = Math.Min(dirPanel.HScrollValue, foregroundPanel.HScrollMaximum);
					foregroundPanel.VScrollValue = Math.Min(dirPanel.VScrollValue, foregroundPanel.VScrollMaximum);
					break;
				case Tab.Swap:
					foregroundPanel.HScrollValue = Math.Min(swapPanel.HScrollValue, foregroundPanel.HScrollMaximum);
					foregroundPanel.VScrollValue = Math.Min(swapPanel.VScrollValue, foregroundPanel.VScrollMaximum);
					objectPanel.HScrollValue = Math.Min(swapPanel.HScrollValue, objectPanel.HScrollMaximum);
					objectPanel.VScrollValue = Math.Min(swapPanel.VScrollValue, objectPanel.VScrollMaximum);
					dirPanel.HScrollValue = Math.Min(swapPanel.HScrollValue, dirPanel.HScrollMaximum);
					dirPanel.VScrollValue = Math.Min(swapPanel.VScrollValue, dirPanel.VScrollMaximum);
					break;
				case Tab.SSZ:
					backgroundPanel.HScrollValue = Math.Min(sszPanel.HScrollValue, backgroundPanel.HScrollMaximum);
					backgroundPanel.VScrollValue = Math.Min(sszPanel.VScrollValue, backgroundPanel.VScrollMaximum);
					break;
				case Tab.Background:
					sszPanel.HScrollValue = Math.Min(sszPanel.HScrollValue, backgroundPanel.HScrollMaximum);
					sszPanel.VScrollValue = Math.Min(sszPanel.VScrollValue, backgroundPanel.VScrollMaximum);
					break;
			}
			loaded = true;
			DrawLevel();
		...
		internal void DrawLevel() {
			...
			switch (CurrentTab) {
				...
				case Tab.Background:
					...
					LevelImg8bpp = LevelData.DrawBackground(dispRect, lowToolStripMenuItem.Checked, highToolStripMenuItem.Checked, path1ToolStripMenuItem.Checked, path2ToolStripMenuItem.Checked, CurrentTab == Tab.SSZ);
					break;
			}
			#endregion
			#region Grid
			...
			switch (CurrentTab) {
				...
				case Tab.Background:
					...
					break;
				case Tab.Directions:
					foreach (DirDat d in LevelData.Directions.list) {
						Rectangle r = Round(d.ToRect());
						Color c;
						if (dispRect.IntersectsWith(r)) {
							r.Offset(-camera.X, -camera.Y);

							if (selecting && selected.Contains(d) && selectmode != 0x81) {
								c = Color.FromArgb(d.color.ToArgb() + 0x20000000);

							} else {
								c = Color.FromArgb(d.color.ToArgb());
							}

							LevelGfx.FillRectangle(new SolidBrush(c), r);
							LevelGfx.DrawRectangle(new Pen(c) { Width = 5, Alignment = PenAlignment.Inset }, r);
						}
					}

					foreach (DirDat d in LevelData.Directions.list) {
						Point p = new Point(d.sxpos, d.sypos);
						Color c = Color.FromArgb(d.color.ToArgb() + 0x40101010);
						Rectangle x = d.ToRectSpawn();
						if (dispRect.IntersectsWith(x)) {

							if (selecting && selected.Contains(d) && selectmode == 0x81) {
								c = Color.FromArgb(d.color.ToArgb() + 0x60101010);
							}

							Rectangle n = d.ToRectSpawn();
							n.Offset(-camera.X, -camera.Y);
							LevelGfx.FillRectangle(new SolidBrush(c), n);

						}

						p.Offset(-camera.X, -camera.Y);
						if (dispRect.IntersectsWith(Round(d.ToRect())) || dispRect.IntersectsWith(x)) {
							if (selecting && selected.Contains(d) && selectmode == 0x81) {
								c = Color.FromArgb(d.color.ToArgb() + 0x60101010);
							}

							Point s = Round(d.Middle());
							s.Offset(-camera.X, -camera.Y);
							LevelGfx.DrawLine(new Pen(c) { Width = 5, Alignment = PenAlignment.Inset }, p, s);
						}
					}

					if (selecting && selected != null && selectmode != 0x81) {
						Image i = LoadRotatedImage(GetImage(selected[0].angle).To32bpp().Resize(new Size(64, 64)), ((selected[0].angle & 0xFF) ^ 0x80) / 255F * 360);
						Point p = Round(selected[0].Middle());
						p.Offset(-32, -32);
						p.Offset(-camera.X, -camera.Y);
						LevelGfx.DrawImage(i, p);
						i.Dispose();
					}
					break;
				case Tab.Swap:
					foreach (Swaps s in LevelData.Swaps.subs)
						if (dispRect.IntersectsWith(s.toRect()))
							foreach (SwapDat d in s.items) {
								...
								if (dispRect.IntersectsWith(r)) {
									...
									if (selecting && selected2 == d && selectmode != 0x81) {
										c = Color.FromArgb(d.color.ToArgb() + 0x20000000);
					...
					/*	if (selecting && selected != null && selectmode != 0x81 && (selected.angle < 0x8100)) {
							Image i = LoadRotatedImage(((selected.angle >= 0x8000) ? SonLVL.Properties.Resources.nodir_arrow : SonLVL.Properties.Resources.dir_arrow).To32bpp().Resize(new Size(64, 64)), ((selected.angle & 0xFF) ^ 0x80) / 255F * 360);
							Point p = round(selected.Middle());
							p.Offset(-32, -32);
							p.Offset(-camera.X, -camera.Y);
							LevelGfx.DrawImage(i, p);
							i.Dispose();
					...
					break;
				case Tab.SSZ:
					foreach (SSZdat d in LevelData.SSZ.list) {
						Rectangle r = d.ToRect();
						Color c;
						if (dispRect.IntersectsWith(r)) {
							r.Offset(-camera.X, -camera.Y);

							if (selecting && selectedssz == d) {
								c = Color.FromArgb(SSZ.color + 0x20000000);

							} else {
								c = Color.FromArgb(SSZ.color);
							}
		...
		private Bitmap GetImage(int angle) {
			if (angle >= 0xC000) {
				// no dir
				return (angle & 0x100) == 0 ? Properties.Resources.down_arrow2_white : (angle & 0x200) == 0 ? Properties.Resources.down_arrow2_red : Properties.Resources.down_arrow2_yel;

			} else if (angle >= 0x8000) {
				// lazy cam
				return (angle & 0x100) == 0 ? Properties.Resources.down_arrow_white : (angle & 0x200) == 0 ? Properties.Resources.down_arrow_red : Properties.Resources.down_arrow_yel;

			} else {
				// normal cam
				return (angle & 0x100) == 0 ? Properties.Resources.down_arrow1_white : (angle & 0x200) == 0 ? Properties.Resources.down_arrow1_red : Properties.Resources.down_arrow1_yel;
			}
		}
		...
		private Point Round(Point p) {
			p.X = 8 * (int)Math.Round(p.X / 8D);
			p.Y = 8 * (int)Math.Round(p.Y / 8D);
			return p;
		}

		private Rectangle Round(Rectangle r) {
		...
		private Bitmap LoadRotatedImage(Bitmap img, float angle) {
		...
		private void UpdateScrollBars() {
			...
			dirPanel.HScrollMaximum = (int)Math.Max(((LevelData.FGWidth + 1) * LevelData.Level.ChunkWidth) - (dirPanel.PanelWidth / ZoomLevel), 0);
			dirPanel.VScrollMaximum = (int)Math.Max(((LevelData.FGHeight + 1) * LevelData.Level.ChunkHeight) - (dirPanel.PanelHeight / ZoomLevel), 0);
			swapPanel.HScrollMaximum = (int)Math.Max(((LevelData.FGWidth + 1) * LevelData.Level.ChunkWidth) - (swapPanel.PanelWidth / ZoomLevel), 0);
			swapPanel.VScrollMaximum = (int)Math.Max(((LevelData.FGHeight + 1) * LevelData.Level.ChunkHeight) - (swapPanel.PanelHeight / ZoomLevel), 0);
			sszPanel.HScrollMaximum = (int)Math.Max(((LevelData.BGWidth + 1) * LevelData.Level.ChunkWidth) - (sszPanel.PanelWidth / ZoomLevel), 0);
			sszPanel.VScrollMaximum = (int)Math.Max(((LevelData.BGHeight + 1) * LevelData.Level.ChunkHeight) - (sszPanel.PanelHeight / ZoomLevel), 0);
		}
		#endregion

		#region Directions
		private void dirPanel_KeyDown(object sender, KeyEventArgs e) {
			...
			switch (e.KeyCode) {
				case Keys.Up:
					if (!loaded)
						return;
					dirPanel.VScrollValue = (int)Math.Max(dirPanel.VScrollValue - vstep, dirPanel.VScrollMinimum);
					break;
				case Keys.Down:
					if (!loaded)
						return;
					dirPanel.VScrollValue = (int)Math.Min(dirPanel.VScrollValue + vstep, dirPanel.VScrollMaximum - LevelData.Level.ChunkHeight + 1);
					break;
				case Keys.Left:
					if (!loaded)
						return;
					dirPanel.HScrollValue = (int)Math.Max(dirPanel.HScrollValue - hstep, dirPanel.HScrollMinimum);
					break;
				case Keys.Right:
					if (!loaded)
						return;
					dirPanel.HScrollValue = (int)Math.Min(dirPanel.HScrollValue + hstep, dirPanel.HScrollMaximum - LevelData.Level.ChunkWidth + 1);
					break;
				...
				case Keys.Delete:
					...
					if (selected != null) {
						LevelData.Directions.list.Remove(selected[0]);
						dirlist.Items.Remove(selected[0].Item);
						DrawLevel();
					}
					break;
				...
				case Keys.A:
					if (selecting && selected != null) {
						int a = selected[0].angle & 0xFF00;
						selected[0].angle += 0x20;
						selected[0].angle &= 0xFF;
						selected[0].angle |= a;
						DrawLevel();
						UpdateDirItemText(selected[0]);
					}
					break;
				...
				case Keys.Q:
					if (selecting && selected != null) {
						if ((selected[0].angle & 0xC000) < 0x8000) {
							selected[0].angle |= 0x8000;

						} else if ((selected[0].angle & 0xC000) == 0x8000) {
							selected[0].angle |= 0xC000;

						} else {
							selected[0].angle &= 0x0FFF;
						}
				...
				case Keys.R:
					if (selecting && selected != null) {
						if ((selected[0].angle & 0x0300) == 0) {
							selected[0].angle |= 0x0100;

						} else if ((selected[0].angle & 0x0300) == 0x0100) {
							selected[0].angle |= 0x0200;

						} else {
							selected[0].angle &= 0xFCFF;
						}
				...
				case Keys.V: {
						List<DirDat> sl = new List<DirDat>();
						Point mouse = new Point((int)(dirPanel.PointToClient(MousePosition).X / ZoomLevel) + dirPanel.HScrollValue, (int)(dirPanel.PointToClient(MousePosition).Y / ZoomLevel) + dirPanel.VScrollValue);
						Point centre = selectmode == 0x81 ? new Point(selected[0].sxpos, selected[0].sypos) : mouse;
						Rectangle r = new Rectangle(mouse.X - 16, mouse.Y - 22, 32, 44);

						foreach (DirDat d in LevelData.Directions.list) {
							if (d.ToRectSpawn().IntersectsWith(r)) {
								sl.Add(d);
								d.sxpos = centre.X;
								d.sypos = centre.Y;
							}
						...
						if (sl.Count > 0) {
							...
							BrightenColor(selected[0], 0x30);
							dirlist.SelectedItem = selected[0].Item;
							dirlist.EnsureVisible(dirlist.Items.IndexOf(selected[0].Item));
							dirlist.Invalidate();
							DrawLevel();
						}
					}
		...
		private void dirPanel_MouseDown(object sender, MouseEventArgs e) {
			if (!loaded)
				return;
			Point mouse = new Point((int)(e.X / ZoomLevel) + dirPanel.HScrollValue, (int)(e.Y / ZoomLevel) + dirPanel.VScrollValue);
			switch (e.Button) {
				case MouseButtons.Middle:
					AddDirDat(mouse, dirlist.SelectedItem);
					break;

				case MouseButtons.Left:
					if (e.Clicks == 2) {
						AddDirDat(mouse, null);
						break;
					}

					// used for invalidate checking
					var oldmode = new { selecting, selectmode, selected };
					...
					foreach (DirDat d in LevelData.Directions.list) {
						...
						if (r.Contains(mouse)) {
							...
							if (r.Contains(mouse)) {
								...
								Point m = d.Middle();
								switch ((mouse.X > m.X ? 1 : 0) | (mouse.Y > m.Y ? 2 : 0)) {
									case 0: { // top left
											int xd = mouse.X - d.xpos, yd = mouse.Y - d.ypos;
											if (xd < 15 && yd < 15) {
												selectmode = 1 | 4;
											...
											selectmode = xd < yd ? 1 : 4;
											Cursor.Current = xd < yd ? Cursors.SizeWE : Cursors.SizeNS;
										}
										break;

									case 1: { // top right
											int xd = (d.xpos + d.xrad) - mouse.X, yd = mouse.Y - d.ypos;
											if (xd < 15 && yd < 15) {
												selectmode = 2 | 4;
											...
											selectmode = xd < yd ? 2 : 4;
											Cursor.Current = xd < yd ? Cursors.SizeWE : Cursors.SizeNS;
										}
										break;

									case 2: { // bottom left
											int xd = mouse.X - d.xpos, yd = (d.ypos + d.yrad) - mouse.Y;
											if (xd < 15 && yd < 15) {
												selectmode = 1 | 8;
												Cursor.Current = Cursors.SizeNESW;
												break;
											...
											selectmode = xd < yd ? 1 : 8;
											Cursor.Current = xd < yd ? Cursors.SizeWE : Cursors.SizeNS;
										}
										break;

									case 3: { // bottom right
											int xd = (d.xpos + d.xrad) - mouse.X, yd = (d.ypos + d.yrad) - mouse.Y;
											if (xd < 15 && yd < 15) {
												selectmode = 2 | 8;
												Cursor.Current = Cursors.SizeNWSE;
												break;
					...
					DirDat od = oldmode.selected?[0], nd = selected?[0];
					if (oldmode.selected != selected || oldmode.selecting != selecting || oldmode.selectmode != selectmode) {
						bool inv = false;

						if (!selecting) dirlist.SelectedItem = null;
						else {
							BrightenColor(selected[0], 0x30);
							dirlist.SelectedItem = selected[0].Item;
							dirlist.EnsureVisible(dirlist.Items.IndexOf(selected[0].Item));
						...
						if (oldmode.selecting) {
							BrightenColor(oldmode.selected[0], 0x20);
							inv = true;
						}

						if (inv) dirlist.Invalidate();
				...
				case MouseButtons.Right:
					if (e.Clicks == 2) {
						...
						selected = null;
						foreach (DirDat d in LevelData.Directions.list) {
							if (d.ToRect().Contains(mouse)) selected = new DirDat[] { d };
						}
		...
		private void dirPanel_MouseMove(object sender, MouseEventArgs e) {
			if (!loaded)
				return;
			if (e.X < 0 || e.Y < 0 || e.X > dirPanel.PanelWidth || e.Y > dirPanel.PanelHeight)
				return;
			Point mouse = new Point((int)(e.X / ZoomLevel) + dirPanel.HScrollValue, (int)(e.Y / ZoomLevel) + dirPanel.VScrollValue);
			bool draw = false;
			switch (e.Button) {
				case MouseButtons.Left:
					if (objdrag && selected != null) {
						draw = true;
						int mvx = (mouse.X - lastmouse.X), mvy = (mouse.Y - lastmouse.Y);

						foreach (DirDat d in selected)
							switch (selectmode) {
								case 0x80:
									d.xpos += mvx;
									d.ypos += mvy;
								...
								case 0x81:
									d.sxpos += mvx;
									d.sypos += mvy;
									if (d.sxpos < 0) d.sxpos = 0;
									if (d.sypos < 0) d.sypos = 0;

									if (selected.Length <= 1) UpdateDirItemText(d);
									break;

								default:
									if ((selectmode & 1) != 0) {
										// grabbed on left side
										if (d.xrad - mvx < 30) {
											DrawLevel();
											UpdateDirItemText(d);
											return;
									...
									} else if ((selectmode & 2) != 0) {
										// grabbed on right side
										if (d.xrad + mvx < 30) {
											DrawLevel();
											UpdateDirItemText(d);
											return;
		...
		private void dirPanel_MouseUp(object sender, MouseEventArgs e) {
			switch (e.Button) {
				case MouseButtons.Left:
					objdrag = false;
					DrawLevel();
					if (selected != null) UpdateDirItemText(selected[0]);
					break;
				case MouseButtons.Right:
					break;
			}
		...
		private void AddDirDat(Point mouse, ListViewItem item) {
			DirDat d = new DirDat(mouse.X - 0x40, mouse.Y - 0x40, 0x80, 0x80, 0x40, mouse.X, mouse.Y, Dir.GetColor());

			bool upd = false;
			int indx = dirlist.Items.IndexOf(item);
			if (item != null && indx != -1) {
				indx++;
				LevelData.Directions.list.Insert(indx, d);
				upd = true;

			} else {
				indx = LevelData.Directions.list.Count;
				LevelData.Directions.list.Add(d);
			}
			...
			AddDirItem(d, indx);
			BrightenColor(d, 0x40);
			Cursor.Current = Cursors.Arrow;
			if (upd) UpdateDirListQuick();
			DrawLevel();
		...
		private void Tit_Tick(object sender, EventArgs e) {
			((Timer)sender).Dispose();
			if (selected != null) BrightenColor(selected[0], 0x20);
		}

		private void UpdateDirItemText(DirDat d) {
			d.Item.SubItems.Clear();
			d.Item.SubItems.AddRange(new ListViewItem.ListViewSubItem[] {
				new ListViewItem.ListViewSubItem(d.Item, Round(d.xpos).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, Round(d.ypos).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, Round(d.xrad).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, Round(d.yrad).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, (d.angle & 0xFFFF).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, (d.sxpos & 0xFFFF).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, (d.sypos & 0xFFFF).ToString("X4")),
			});

			d.Item.Text = LevelData.Directions.list.IndexOf(d) + "";
			d.Item.BackColor = d.ItemColor;
			int i = dirlist.Items.IndexOf(d.Item);
			if (i != -1) dirlist.RedrawItems(i, i, false);
		}

		private void AddDirItem(DirDat d, int indx) {
			Invoke(new MethodInvoker(delegate () {
				ListViewItem x = new ListViewItem();
				BrightenColor(d, 0x20);
				dirlist.Items.Insert(indx, d.Item = x);
				UpdateDirItemText(d);
			}));
		}
		...
		private void UpdateDirList() {
			dirlist.Items.Clear();
			int i = 0;
			//	LevelData.Directions.list.Reverse();
			foreach (DirDat d in LevelData.Directions.list) {
				if (d.Item == null || !dirlist.Items.Contains(d.Item)) {
					AddDirItem(d, i);
					i++;
				}
			}
		...
		private void UpdateDirListQuick() {
			int i = 0;
			//	LevelData.Directions.list.Reverse();
			foreach (DirDat d in LevelData.Directions.list) {
				if (d.Item == null || !dirlist.Items.Contains(d.Item)) {
					AddDirItem(d, dirlist.Items.Count);
		...
		private void BrightenColor(DirDat d, int amount) {
			if (d == null || d.Item == null) return;

			int R = d.color.R + amount, G = d.color.G + amount, B = d.color.B + amount;
			...
			d.ItemColor = d.Item.BackColor = Color.FromArgb(R, G, B);
		}

		private void dirlist_Hover(object sender, ListViewItemMouseHoverEventArgs e) {
			foreach (DirDat d in LevelData.Directions.list) {
				if (d.Item == e.Item) {

					if (selected != null) BrightenColor(selected[0], 0x20);
					selected = new DirDat[] { d };
					if (selectmode == 0) selectmode = 0x80;
					selecting = true;
					BrightenColor(selected[0], 0x30);
					DrawLevel();
					return;
		...
		private void dirlist_ItemSelection(object sender, ListViewItemSelectionChangedEventArgs e) {
			foreach (DirDat d in LevelData.Directions.list) {
				if (d.Item == e.Item) {
					selected = new DirDat[] { d };
					selecting = true;
		...
		private void dirlist_ItemDragDrop(object sender, API.ListViewItemDragEventArgs e) {
			List<DirDat> old = LevelData.Directions.EmptyList();

			foreach (ListViewItem l in dirlist.Items) {
				DirDat f = null;
				foreach (DirDat d in old) {
					if (d.Item == l) {
						f = d;
						break;
					}
				...
				old.Remove(f);
				LevelData.Directions.list.Add(f);
			}
		...
		private void dirlist_ChkDrop(object sender, API.ListViewItemDragEventArgs e) {
			e.Cancel = dirlist.SelectedIndices.Count > 1;
		}
		...
		private void TextBox1_KeyPress(object sender, KeyPressEventArgs e) {
			if (e.KeyChar == (char)Keys.Return) {
				...
				if (Int32.TryParse(textBox1.Text, out int val)) {
					List<DirDat> l = new List<DirDat>();

					int pos = val;
					foreach (int i in dirlist.SelectedIndices) {
						l.Add(LevelData.Directions.list.ElementAt(i));
					}

					//	l.Reverse();

					foreach (DirDat d in l) {
						LevelData.Directions.list.Remove(d);
					}

					if (val < LevelData.Directions.list.Count) {
						DirDat x = LevelData.Directions.list.ElementAt((val > 0) ? val : 0);

						foreach (DirDat d in l) {
							LevelData.Directions.list.Insert(LevelData.Directions.list.IndexOf(x), d);
						}
		...
		#region Swaps
		private void swap_KeyDown(object sender, KeyEventArgs e) {
			long hstep = e.Control ? int.MaxValue : e.Shift ? LevelData.Level.ChunkWidth : 16;
			long vstep = e.Control ? int.MaxValue : e.Shift ? LevelData.Level.ChunkHeight : 16;
			switch (e.KeyCode) {
				case Keys.Up:
					if (!loaded)
						return;
					swapPanel.VScrollValue = (int)Math.Max(swapPanel.VScrollValue - vstep, swapPanel.VScrollMinimum);
					break;
				case Keys.Down:
					if (!loaded)
						return;
					swapPanel.VScrollValue = (int)Math.Min(swapPanel.VScrollValue + vstep, swapPanel.VScrollMaximum - LevelData.Level.ChunkHeight + 1);
					break;
				case Keys.Left:
					if (!loaded)
						return;
					swapPanel.HScrollValue = (int)Math.Max(swapPanel.HScrollValue - hstep, swapPanel.HScrollMinimum);
					break;
				case Keys.Right:
					if (!loaded)
						return;
					swapPanel.HScrollValue = (int)Math.Min(swapPanel.HScrollValue + hstep, swapPanel.HScrollMaximum - LevelData.Level.ChunkWidth + 1);
					break;
				...
				case Keys.Delete:
					...
					if (selected2 != null) {
						int sub = 0;
						foreach (Swaps s in LevelData.Swaps.subs) {
							if (s.items.Contains(selected2)) {
								s.items.Remove(selected2);
								swaplist.Items.Remove(selected2.Item);
								swapbox.Text = "0x" + sub.ToString("X2");
								DrawLevel();
								break;
							}
							sub++;
				...
				case Keys.A:
					if (selecting && selected2 != null) {
						...
						sub--;
						sub &= 0xFF;
						while (LevelData.Swaps.subs.Count <= sub)
							LevelData.Swaps.subs.Add(new Swaps());

						LevelData.Swaps.subs[sub].items.Add(selected2);
						LevelData.Swaps.subs[sub].refresh();
		...
		private void swap_MouseDown(object sender, MouseEventArgs e) {
			if (!loaded) return;
			Point mouse = new Point((int)(e.X / ZoomLevel) + swapPanel.HScrollValue, (int)(e.Y / ZoomLevel) + swapPanel.VScrollValue);
			switch (e.Button) {
				case MouseButtons.Middle:
					AddSwap(mouse, swaplist.SelectedItem);
					break;
					...
					foreach (Swaps s in LevelData.Swaps.subs) {
						foreach (SwapDat d in s.items) {
							...
							if (r.Contains(mouse)) {
								...
								selected2 = d;
								objdrag = true;
								swaplist.SelectedItem = d.Item;
								swaplist.EnsureVisible(swaplist.Items.IndexOf(d.Item));
								swaplist.Invalidate();
								...
								if (r.Contains(mouse)) {
									...
									switch ((mouse.X > m.X ? 1 : 0) | (mouse.Y > m.Y ? 2 : 0)) {
										case 0: { // top left
												int xd = mouse.X - d.left, yd = mouse.Y - d.up;
												if (xd < 15 && yd < 15) {
													selectmode = 1 | 4;
													Cursor.Current = Cursors.SizeNWSE;
													break;
										...
										case 1: { // top right
												int xd = d.right - mouse.X, yd = mouse.Y - d.up;
												if (xd < 15 && yd < 15) {
													selectmode = 2 | 4;
													Cursor.Current = Cursors.SizeNESW;
													break;
										...
										case 2: { // bottom left
												int xd = mouse.X - d.left, yd = d.down - mouse.Y;
												if (xd < 15 && yd < 15) {
													selectmode = 1 | 8;
													Cursor.Current = Cursors.SizeNESW;
													break;
										...
										case 3: { // bottom right
												int xd = d.right - mouse.X, yd = d.down - mouse.Y;
												if (xd < 15 && yd < 15) {
													selectmode = 2 | 8;
													Cursor.Current = Cursors.SizeNWSE;
													break;
				...
				case MouseButtons.Right:
					if (e.Clicks == 2) {
						...
						foreach (Swaps s in LevelData.Swaps.subs) {
							foreach (SwapDat d in s.items) {
								if (d.toRect().Contains(mouse)) selected2 = d;
							}
						}
		...
		private void swap_MouseMove(object sender, MouseEventArgs e) {
			if (!loaded) return;
			if (e.X < 0 || e.Y < 0 || e.X > swapPanel.PanelWidth || e.Y > swapPanel.PanelHeight)
				return;
			Point mouse = new Point((int)(e.X / ZoomLevel) + swapPanel.HScrollValue, (int)(e.Y / ZoomLevel) + swapPanel.VScrollValue);
			bool draw = false;
			switch (e.Button) {
				case MouseButtons.Left:
					if (objdrag && selected2 != null) {
						draw = true;
						int mvx = (mouse.X - lastmouse.X), mvy = (mouse.Y - lastmouse.Y);
						...
						switch (selectmode) {
							case 0x80:
								...
								if (selected2.left < 0) {
									selected2.right -= selected2.left;
									selected2.left = 0;
								}

								if (selected2.up < 0) {
									selected2.down -= selected2.up;
									selected2.up = 0;
								}

								UpdateSwapItemText(selected2);
								SwapRef(selected2);
								break;
							...
							default:
								if ((selectmode & 1) != 0) {
									// grabbed on left side
									if (selected2.right - selected2.left - mvx < 30) {
										DrawLevel();
										UpdateSwapItemText(selected2);
										return;
									}
								...
								if ((selectmode & 4) != 0) {
									// grabbed on top
									if (selected2.down - selected2.up - mvy < 30) {
										DrawLevel();
										UpdateSwapItemText(selected2);
										return;
									}
		...
		private void AddSwap(Point mouse, ListViewItem item) {
			SwapDat d = new SwapDat(mouse.X - 0x40, mouse.X + 0x40, mouse.Y - 0x40, mouse.Y + 0x40, Dir.GetColor());

			bool upd = false;
			int indx = swaplist.Items.IndexOf(item), sub = 0;
			if (item != null && indx != -1) {
				indx++;
				upd = true;
				try {
					sub = Convert.ToInt32(item.Text, 16);
				} catch (Exception) {
					try {
						if (item.Text.StartsWith("0x")) sub = Convert.ToInt32(swapbox.Text.Substring(2), 16);
						else sub = Convert.ToInt32(swapbox.Text, 10);
				...
				try {
					if (swapbox.Text.StartsWith("0x")) sub = Convert.ToInt32(swapbox.Text.Substring(2), 16);
					else sub = Convert.ToInt32(swapbox.Text, 10);
				...
				indx = swaplist.Items.Count;
			}

			while (LevelData.Swaps.subs.Count <= sub)
				LevelData.Swaps.subs.Add(new Swaps());
			LevelData.Swaps.subs[sub].items.Add(d);
			LevelData.Swaps.subs[sub].refresh();
			...
			AddSwapItem(d, indx, sub);
			BrightenColor(d, 0x20);
			Cursor.Current = Cursors.Arrow;
			if (upd) UpdateSwapList();
			DrawLevel();
		...
		private void BrightenColor(SwapDat d, int amount) {
			if (d == null || d.Item == null) return;
		...
		private void SwapRef(SwapDat d) {
			foreach (Swaps s in LevelData.Swaps.subs) {
				if (s.items.Contains(d)) {
					s.refresh();
					break;
				}
			}
		...
		private void swap_MouseUp(object sender, MouseEventArgs e) {
			...
			switch (e.Button) {
				case MouseButtons.Left:
					objdrag = false;
					DrawLevel();
					if (selected2 != null) UpdateSwapItemText(selected2);
					break;
				case MouseButtons.Right:
					break;
			}
		...
		private void UpdateSwapItemText(SwapDat d) {
			int sub = 0;
			foreach (Swaps s in LevelData.Swaps.subs) {
				if (s.items.Contains(d)) {
					UpdateSwapItemText(d, sub);
					return;
				}
				sub++;
			}
		...
		private void UpdateSwapItemText(SwapDat d, int sub) {
			d.Item.SubItems.Clear();
			d.Item.SubItems.AddRange(new ListViewItem.ListViewSubItem[] {
				new ListViewItem.ListViewSubItem(d.Item, Round(d.left).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, Round(d.right).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, Round(d.up).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, Round(d.down).ToString("X4")),
			});

			d.Item.Text = sub.ToString("X2") + "";
			d.Item.BackColor = d.ItemColor;
			int i = swaplist.Items.IndexOf(d.Item);
			if (i != -1) swaplist.RedrawItems(i, i, false);
		}

		private void AddSwapItem(SwapDat d, int indx, int sub) {
			ListViewItem x = new ListViewItem();
			swaplist.Items.Insert(indx, d.Item = x);
			UpdateSwapItemText(d, sub);
			BrightenColor(d, 0x20);
		}
		...
		private void UpdateSwapList() {
			...
			foreach (Swaps s in LevelData.Swaps.subs) {
				foreach (SwapDat d in s.items)
					if (d.Item == null || !swaplist.Items.Contains(d.Item)) {
						AddSwapItem(d, i, sub);
						i++;
					}
				sub++;
		...
		private void swap_ItemSelection(object sender, ListViewItemSelectionChangedEventArgs e) {
			int sub = 0;
			foreach (Swaps s in LevelData.Swaps.subs) {
				foreach (SwapDat d in s.items) {
					if (d.Item == e.Item) {
		...
		#region SSZ
		private void sszPanel_KeyDown(object sender, KeyEventArgs e) {
			long hstep = e.Control ? int.MaxValue : e.Shift ? LevelData.Level.ChunkWidth : 16;
			long vstep = e.Control ? int.MaxValue : e.Shift ? LevelData.Level.ChunkHeight : 16;
			...
			switch (e.KeyCode) {
				case Keys.Up:
					if (!loaded) return;
					sszPanel.VScrollValue = (int)Math.Max(sszPanel.VScrollValue - vstep, sszPanel.VScrollMinimum);
					break;
				case Keys.Down:
					if (!loaded) return;
					sszPanel.VScrollValue = (int)Math.Min(sszPanel.VScrollValue + vstep, sszPanel.VScrollMaximum - LevelData.Level.ChunkHeight + 1);
					break;
				case Keys.Left:
					if (!loaded) return;
					sszPanel.HScrollValue = (int)Math.Max(sszPanel.HScrollValue - hstep, sszPanel.HScrollMinimum);
					break;
				case Keys.Right:
					if (!loaded) return;
					sszPanel.HScrollValue = (int)Math.Min(sszPanel.HScrollValue + hstep, sszPanel.HScrollMaximum - LevelData.Level.ChunkWidth + 1);
					break;
				...
				case Keys.Delete:
					...
					if (selectedssz != null) {
						LevelData.SSZ.list.Remove(selectedssz);
						sszlist.Items.Remove(selectedssz.Item);
						DrawLevel();
					}
					break;
				case Keys.V:
					if (!loaded) return;
					if (selecting && selectedssz != null) {
						selectedssz.flags ^= 1;
						UpdateSSZItemText(selectedssz);
						selectedssz.drawBits(LevelData.SSZ);
						DrawLevel();
					}
					break;
			}
		...
		private void sszPanel_MouseDown(object sender, MouseEventArgs e) {
			if (!loaded) return;
			Point mouse = new Point((int)(e.X / ZoomLevel) + sszPanel.HScrollValue, (int)(e.Y / ZoomLevel) + sszPanel.VScrollValue);

			switch (e.Button) {
				case MouseButtons.Middle:
					AddSSZ(mouse, sszlist.SelectedItem);
					break;

				case MouseButtons.Left:
					if (e.Clicks == 2) {
						AddSSZ(mouse, null).drawBits(LevelData.SSZ);
						DrawLevel();
						break;
					}
					...
					foreach (SSZdat d in LevelData.SSZ.list) {
						...
						if (r.Contains(mouse)) {
							...
							selectedssz = d;
							objdrag = true;
							surfacetext.Text = d.surface.ToString("X4");
							sszlist.SelectedItem = d.Item;
							sszlist.EnsureVisible(sszlist.Items.IndexOf(d.Item));
							sszlist.Invalidate();
							...
							if (r.Contains(mouse)) {
								...
								Point m = d.Middle();
								selectmode = mouse.X > m.X ? 2 : 1;
								Cursor.Current = Cursors.SizeWE;
							}
						}
					}

					if (!selecting) sszlist.SelectedItem = null;
					DrawLevel();
					break;
				...
				case MouseButtons.Right:
					if (e.Clicks == 2) {
						...
						foreach (SSZdat d in LevelData.SSZ.list) {
							if (d.ToRect().Contains(mouse)) selectedssz = d;
						}
		...
		private void sszPanel_MouseMove(object sender, MouseEventArgs e) {
			if (!loaded) return;
			if (e.X < 0 || e.Y < 0 || e.X > sszPanel.PanelWidth || e.Y > sszPanel.PanelHeight)
				return;
			...
			switch (e.Button) {
				case MouseButtons.Left:
					if (objdrag && selectedssz != null) {
						draw = true;
						int mvx = (mouse.X - lastmouse.X), mvy = (mouse.Y - lastmouse.Y);

						switch (selectmode) {
							case 0x80:
								selectedssz.xpos += mvx;
								selectedssz.ypos += mvy;
								if (selectedssz.xpos < 0) selectedssz.ypos = 0;
								if (selectedssz.ypos < 0) selectedssz.ypos = 0;
								UpdateSSZItemText(selectedssz);
								break;
							...
							default:
								if ((selectmode & 1) != 0) {
									// grabbed on left side
									if (selectedssz.width - mvx < 30) {
										DrawLevel();
										UpdateSSZItemText(selectedssz);
										return;
									}

									selectedssz.width -= mvx / 2;
									selectedssz.xpos += mvx / 2;

									if (selectedssz.xpos < 0) {
										selectedssz.width -= selectedssz.xpos;
										selectedssz.xpos = 0;
									}
		...
		private void sszPanel_MouseUp(object sender, MouseEventArgs e) {
			if (!loaded) return;
		...
		private SSZdat AddSSZ(Point mouse, ListViewItem item) {
			SSZdat d = new SSZdat(0, mouse.X - 0x40, mouse.Y - 0x140, 0x40, 0);

			bool upd = false;
			int indx = sszlist.Items.IndexOf(item), sub = 0;
			if (item != null && indx != -1) {
				indx++;
				upd = true;

			} else indx = sszlist.Items.Count;

			LevelData.SSZ.list.Add(d);
			selecting = false;
			selectedssz = d;
			surfacetext.Text = d.surface.ToString("X4");
			...
			AddSSZItem(d, indx);
			Cursor.Current = Cursors.Arrow;
			if (upd) UpdateSSZList();
			DrawLevel();
			return d;
		}

		private void UpdateSSZItemText(SSZdat d) {
			d.Item.SubItems.Clear();
			d.Item.SubItems.AddRange(new ListViewItem.ListViewSubItem[] {
				new ListViewItem.ListViewSubItem(d.Item, (d.flags & 0xFFFF).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, SSZ.Round32(d.xpos).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, SSZ.Round64(d.ypos).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, SSZ.Round32(d.width).ToString("X4")),
				new ListViewItem.ListViewSubItem(d.Item, (d.surface & 0xFFFF).ToString("X4")),
			});

			d.Item.Text = LevelData.SSZ.list.IndexOf(d) + "";
			d.Item.BackColor = Color.FromArgb(SSZ.color & 0xFFFFFF);
			int i = sszlist.Items.IndexOf(d.Item);
			if (i != -1) sszlist.RedrawItems(i, i, false);
		}

		private void UpdateSSZList() {
			sszlist.Items.Clear();
			int i = 0;
			//	LevelData.Directions.list.Reverse();
			foreach (SSZdat d in LevelData.SSZ.list) {
				if (d.Item == null || !sszlist.Items.Contains(d.Item)) {
					AddSSZItem(d, i);
					i++;
				}
			}
		...
		private void AddSSZItem(SSZdat d, int indx) {
			ListViewItem x = new ListViewItem();
			sszlist.Items.Insert(indx, d.Item = x);
			UpdateSSZItemText(d);
		}
		#endregion
		...
		private void T_Tick(object sender, EventArgs e) {
			if (this.InvokeRequired) {
				this.BeginInvoke(new EventHandler(T_Tick), sender, e);
		...
		private void backgroundLevelLoader_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			...
			dirPanel.HScrollValue = 0;
			dirPanel.HScrollSmallChange = 16;
			dirPanel.HScrollLargeChange = 128;
			dirPanel.VScrollValue = 0;
			dirPanel.VScrollSmallChange = 16;
			dirPanel.VScrollLargeChange = 128;
			dirPanel.HScrollEnabled = true;
			dirPanel.VScrollEnabled = true;
			swapPanel.HScrollValue = 0;
			swapPanel.HScrollSmallChange = 16;
			swapPanel.HScrollLargeChange = 128;
			swapPanel.VScrollValue = 0;
			swapPanel.VScrollSmallChange = 16;
			swapPanel.VScrollLargeChange = 128;
			swapPanel.HScrollEnabled = true;
			swapPanel.VScrollEnabled = true;
			sszPanel.HScrollValue = 0;
			sszPanel.HScrollSmallChange = 16;
			sszPanel.HScrollLargeChange = 128;
			sszPanel.VScrollValue = 0;
			sszPanel.VScrollSmallChange = 16;
			sszPanel.VScrollLargeChange = 128;
			sszPanel.HScrollEnabled = true;
			sszPanel.VScrollEnabled = true;
					...
					for (int i = 0; i < LevelData.Blocks.Count; i++)
					{
						...
						if (exportArtcollisionpriorityToolStripMenuItem.Checked)
						{
							...
							switch (LevelData.Level.ChunkFormat)
							{
								...
								case EngineVersion.S3K:
								case EngineVersion.SKC:
									dualPath = !System.Object.ReferenceEquals(LevelData.ColInds1, LevelData.ColInds2);
									break;
							}
							if (dualPath)
							{
			...
			using (SaveFileDialog a = new SaveFileDialog()
				...
				if (a.ShowDialog() == DialogResult.OK)
				{
					if (exportArtcollisionpriorityToolStripMenuItem.Checked)
					{
						string pathBase = Path.Combine(Path.GetDirectoryName(a.FileName), Path.GetFileNameWithoutExtension(a.FileName));
						string pathExt = Path.GetExtension(a.FileName);
						BitmapBits bmp = LevelData.DrawBackground(null, true, true, false, false, CurrentTab == Tab.SSZ);
						for (int i = 0; i < bmp.Bits.Length; i++)
							if (bmp.Bits[i] == 0)
								bmp.Bits[i] = 32;
						Bitmap res = bmp.ToBitmap();
						...
						if (dualPath)
						{
							bmp = LevelData.DrawBackground(null, false, false, true, false, CurrentTab == Tab.SSZ);
							bmp.UnfixUIColors();
							bmp.ToBitmap4bpp(Color.Magenta, Color.White, Color.Yellow, Color.Black).Save(pathBase + "_col1" + pathExt);
							bmp = LevelData.DrawBackground(null, false, false, false, true, CurrentTab == Tab.SSZ);
							bmp.UnfixUIColors();
							bmp.ToBitmap4bpp(Color.Magenta, Color.White, Color.Yellow, Color.Black).Save(pathBase + "_col2" + pathExt);
						}
					...
					{
						BitmapBits bmp = LevelData.DrawBackground(null, lowToolStripMenuItem.Checked, highToolStripMenuItem.Checked, path1ToolStripMenuItem.Checked, path2ToolStripMenuItem.Checked, CurrentTab == Tab.SSZ);
						using (Bitmap res = bmp.ToBitmap(LevelImgPalette))
							res.Save(a.FileName);
					}
				}
		...
		private void BlockCollision1_ValueChanged(object sender, EventArgs e)
		{
			...
			if (System.Object.ReferenceEquals(LevelData.ColInds1, LevelData.ColInds2))
			...
			for (int i = 0; i < blocks.Count; i++)
			{
				...
				if (blockcoldata != null)
				{
					if (colInds1[i] != col.ColInd1)
						continue;
					if (!System.Object.ReferenceEquals(LevelData.ColInds1, LevelData.ColInds2) && colInds2[i] != col.ColInd2)
						continue;
				}
			...
			for (int i = 0; i < LevelData.Blocks.Count; i++)
			{
				...
				foreach (var item in blocks)
				{
					if (LevelData.ColInds1[i] != LevelData.ColInds1[colIndMap[item.Key]])
						continue;
					if (!System.Object.ReferenceEquals(LevelData.ColInds1, LevelData.ColInds2) && LevelData.ColInds2[i] != LevelData.ColInds2[colIndMap[item.Key]])
						continue;
