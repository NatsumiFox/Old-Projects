...
namespace SonicRetro.SonLVL.API
{
	public static class LevelData
	{
		...
		public static S3K.Dir Directions;
		public static S3K.Swap Swaps;
		public static S3K.SSZ SSZ;
		...
		public static void LoadLevel(string levelname, bool loadGraphics)
		{
			...
			if (Level.DirBox != null && Level.DirPos != null && File.Exists(Level.DirBox) && File.Exists(Level.DirPos)) {
				Directions = new S3K.Dir(File.ReadAllBytes(Level.DirBox), File.ReadAllBytes(Level.DirPos));

			} else {
				Directions = new S3K.Dir();
			}

			if (Level.Swap != null && Level.DirPos != null) {
				Swaps = new S3K.Swap(Level.Swap);

			} else {
				Swaps = new S3K.Swap();
			}

			if (Level.HeaderSSZ != null && Level.ChunkSSZ != null && File.Exists(Level.HeaderSSZ) && File.Exists(Level.ChunkSSZ)) {
				SSZ = new S3K.SSZ(File.ReadAllBytes(Level.HeaderSSZ), File.ReadAllBytes(Level.ChunkSSZ));
				SSZ.InitBits();
			...
			switch (Level.EngineVersion)
			{
				...
				default:
					Parallel.Invoke(SaveLevelTiles, SaveLevelBlocks, SaveLevelChunks, SaveLevelLayout, SaveLevelPalette,
						SaveLevelObjects, SaveLevelRings, SaveLevelExtraObjects, SaveLevelStartPositions,
						SaveLevelColInds, SaveLevelColArr1, SaveLevelColArr2, SaveLevelAngles, SaveNatsumi);
					break;
			}
		}

		private static void SaveNatsumi() {
			if (Directions != null) {
				Directions.list.Reverse();
				Compression.Compress(Directions.SaveBox(), Level.DirBox, CompressionType.Uncompressed);
				Compression.Compress(Directions.SavePos(), Level.DirPos, CompressionType.Uncompressed);
				Directions.list.Reverse();
			}

			if (Swaps != null) {
				Swaps.Save(Level.Swap, CompressionType.Uncompressed);
			}

			if (SSZ != null) {
				Compression.Compress(SSZ.SaveHeader(), Level.HeaderSSZ, CompressionType.Uncompressed);
				Compression.Compress(SSZ.SaveChunk(), Level.ChunkSSZ, CompressionType.Uncompressed);
			}
		}
		...
		public static BitmapBits DrawBackground(Rectangle? section, bool lowPlane, bool highPlane, bool collisionPath1, bool collisionPath2, bool ssz)
		{
			Rectangle bounds;
			if (section.HasValue)
				bounds = section.Value;
			...
			if (ssz) {
				foreach (S3K.SSZdat d in SSZ.list) {
					if (d.BmpBits != null) {
						Rectangle r = d.ToRect();
						LevelImg8bpp.DrawBitmapComposited(d.BmpBits, r.X - bounds.X, r.Y - bounds.Y);
					}
				}
			}
			return LevelImg8bpp;
