@echo off

derecmp.exe kd map256 map256_u
REM ::: automatic recompression of data - disabled by default because it's slow
REM ::: remove "REM" from the lines below to re-enable it
REM derecmp.exe nc artnem_u artnem
REM derecmp.exe kc artkos_u artkos
REM derecmp.exe ec mapeni_u mapeni
REM derecmp.exe ec map16_u map16
REM derecmp.exe kc map256_u map256
REM derecmp.exe ec sslay_u sslayout

ECHO [ BUILD.bat Game Assembler [v0.01a] by SonicVaan]
ECHO ***** ASSEMBLING ROM... PLEASE WAIT... *****    
ECHO ...

asm68k /o op+ /o os+ /o ow+ /o oz+ /o oaq+ /o osq+ /o omq+ /p /o ae- sonic1.asm, s1built.bin

ECHO ...
ECHO ***** DONE! *****
ECHO ...

ECHO ***** PADDING ROM... *****
ECHO ...
rompad.exe s1built.bin 255 0

ECHO ...
ECHO ***** DONE! *****
ECHO ...
ECHO ***** FIXING HEADER OF PADDED ROM *****
ECHO ...

fixheadr.exe s1built.bin
ECHO ...
ECHO ***** DONE! *****
ECHO ...
ECHO ***** ASSEMBLING UNPADDED ROM... PLEASE WAIT... *****   
ECHO ...

asm68k /o op+ /o os+ /o ow+ /o oz+ /o oaq+ /o osq+ /o omq+ /p /o ae- sonic1.asm, s1built_nopad.bin
ECHO ...
ECHO ***** DONE! *****
ECHO ...
ECHO ***** FIXING HEADER OF UNPADDED ROM *****
ECHO ...

fixheadr.exe s1built_nopad.bin
ECHO ...
ECHO ***** DONE! *****
ECHO ...
ECHO HAVE FUN! ;)
pause