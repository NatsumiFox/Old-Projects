...
Pad_Init:
		clr.b	PadPoll.w				; clear poll stat
...
; ------------------------------------------------------------------------------
	...
	pad_delay8						;
		...
		move.w	#$EAEA,Pad1Mode.w			; enable EA 4-way play
		rts
	endif
; ------------------------------------------------------------------------------

.notEA
		lea	Pad1Mode.w,a1				; load pad1 mode to a1
...
.getcode
		...
		bsr.w	Pad_Handshake				; do handshake
...
Pad_Read:
		tst.b	PadPoll.w				; test poll stat
		bne.w	Pad_Init				; if was set, poll controllers again

		lea	$A10003,a0				; load pad1 data to a0
		lea	Pad1AHeld.w,a1				; load pad1a buttons to a1
		lea	Pad1Mode.w,a2				; load pad1 mode to a2
...
.readpad
	...
	if PAD_ENABLE_MULTI
		...
		beq.w	Pad_ReadTeamPlayer			; branch if yes
	endif
...
.mouse
	...
	pad_delay4						;
		...
		move.b	d0,(a0)					; next packet
		bsr.s	Pad_Handshake				; do handshake
		moveq	#$F,d1					; load mask to d1
		and.b	(a0),d1					; check packet value
