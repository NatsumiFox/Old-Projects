...
dPSG =		$C00011		; quick reference to PSG port
	...
	rsset v_snddriver_ram	; Insert your RAM definition here!
...
mTL		rs.b tSize	; TL modulation data
...
MusCount =	$01		; number of installed music tracks
...
dCALC_BANK	macro off
	lea	VoiceBank+\off(pc),a1	; load sound effects voice table into a6
	cmp.w	#mSFXDAC1,a5		; check if this is a SFX channel
	bhs.s	.bank			; if so, branch
	move.l	mVctMus.w,a1		; load music voice table into a1

	if \off<>0
		add.w	#\off,a1		; add offset into a1
	endif
.bank
    endm
; ===========================================================================
...
; ===========================================================================
; ---------------------------------------------------------------------------
; Create data for a special end-looping sample
; ---------------------------------------------------------------------------

samplex		macro freq, start, offset, name
	if narg=4		; if we have 4 arguments, we'd like a custom name
d\name =	__samp		; use the extra argument to create SMPS2ASM equate
	else
d\start =	__samp		; else, use the first one!
...
; create offsets for the sample normal, reverse, loop normal, loop reverse.
	dc.b SWF_\start&$FF,((SWF_\start>>$08)&$7F)|$80,(SWF_\start>>$0F)&$FF
	dc.b (SWFR_\start-1)&$FF,(((SWFR_\start-1)>>$08)&$7F)|$80,((SWFR_\start-1)>>$0F)&$FF
	dc.b ((SWFR_\start)-\offset)&$FF,((((SWFR_\start)-\offset)>>$08)&$7F)|$80, (((SWFR_\start)-\offset)>>$0F)&$FF
	dc.b ((SWF_\start)+\offset)&$FF,((((SWF_\start)+\offset)>>$08)&$7F)|$80,(((SWF_\start)+\offset)>>$0F)&$FF
	dc.w \freq-$100		; sample frequency (actually offset, so we remove $100)
	dc.w 0			; unused!
    endm
; ===========================================================================
