...
.meta
	...
	bra.w	dcCont		; FF 28 - Enable raw frequency mode (RAW_FREQ)
...
tlmod	macro name
	...
	tlmod	dcModOnTL	; FF 4x - Turn on TL Modulation for operator x (TL_MOD - MODS_ON)
	tlmod	dcModOffTL	; FF 5x - Turn off TL Modulation for operator x (TL_MOD - MODS_OFF)
	tlmod	dcModTL		; FF 6x - Modulation for operator x (TL_MOD - MOD_SETUP)
	tlmod	dcVolEnvTL	; FF 7y - Set TL volume envelope to xx for operator y (TL_MOD - FM_VOLENV)
...
; ---------------------------------------------------------------------------
		...
		lea	mTL.w,a2		; load source address to a0
		lea	mBackTL.w,a1		; load destination address to a1
		move.w	#(mBackTL-mTL)/4-1,d0	; load backup size to d0
...
dUpdateVoiceFM:
	dCALC_BANK	0			; get the voice table address to a1
	dCALC_VOICE				; get address of the specific voice to a1
		...
		move.b	(a1)+,(a3)+		; load feedback and algorithm
	...
	endr
		...
		move.l	a4,d4			; save tracker address to stack
		move.l	a6,a4			; copy TL data to a4
		move.l	a6,d6			; save the TL data to stack
...
.tlloop
		...
		bpl.s	.nonslot		; branch if nonslot
		and.w	#$7F,d1			; get rid of sign bit (ugh)
...
.nonslot
		...
		add.w	#toSize,a4		; go to next operator
		dbf	d5,.tlloop		; repeat for each Total Level operator
		move.l	d6,a6			; load TL data back from stack
		move.l	d4,a4			; load tracker address back from stack
...
; ---------------------------------------------------------------------------
	...
	tlmodrt	0, dcModTL			; generate call structure to this routine
		move.l	a4,toMod(a1)		; set modulation data address
		move.b	(a4)+,toModSpeed(a1)	; load modulation speed from tracker to channel

		move.b	(a4)+,d0		; load modulation step count from tracker to d0
		lsr.b	#1,d0			; halve it
		move.b	d0,toModCount(a1)	; save as modulation step count to channel

		move.b	(a4)+,toModDelay(a1)	; load modulation delay from tracker to channel
		move.b	(a4)+,toModStep(a1)	; load modulation step offset from tracker to channel
...
; ---------------------------------------------------------------------------
	...
	tlmodrt	0, dcModOnTL			; generate call structure to this routine
		or.b	#$81,(a1)		; set modulation as enabled and operator active
		or.b	#$40,(a6)		; also enable whole system as operational
		rts
; ===========================================================================
; ---------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------
	...
	tlmodrt	1, dcModOffTL			; generate call structure to this routine
		and.b	#$FE,(a1)		; set modulation as disabled
		rts
; ===========================================================================
; ---------------------------------------------------------------------------
; Tracker command for setting volume envelope ID
; ---------------------------------------------------------------------------
	...
	tlmodrt	1, dcVolEnvTL			; generate call structure to this routine
		move.b	(a4)+,toVolEnv(a1)	; set volume envelope ID
		rts
; ===========================================================================
; ---------------------------------------------------------------------------
...
dcResetVolEnvTL:
		clr.b	toVolEnv(a1)		; reset volume envelope ID
...
dcComplexTL:
		move.b	(a4)+,d1		; load settings value to d1
		move.b	d1,d2			; copy it to d2
		and.w	#$F0,d1			; get only the mode to use

		lea	-toSize(a6),a1		; get the TL list to use
		moveq	#4-1,d3			; set repeat count

.operator
		add.w	#toSize,a1		; get the next TL data to use
		btst	d3,d2			; check if operator is enabled
		beq.s	.disabled		; if not, skip

		lea	dcComplexTable(pc,d1.w),a2; get complex table data to a2
		move.w	(a2)+,d0		; get offset to the routine to run
		jsr	-2(a2,d0.w)		; run first routine
		move.w	(a2),d0			; get offset to the routine to run
		jsr	-2(a2,d0.w)		; run second routine

.disabled
		dbf	d3,.operator		; loop for each operator
		tst.b	2(a2)			; check if we should run extra code
...
dcUpdateTL:
		and.b	#$3F,(a6)		; reset TL modulation enabled bits
		moveq	#$00,d0			; set default state as disabled
		moveq	#4-1,d1			; repeat for 4 channels

		move.l	a6,a1			; copy pointer to a1
		moveq	#toSize,d2		; load channel size to d2

.chloop
		and.b	#$7F,(a1)		; set as active
		btst	#0,(a1)			; check if modulation is active
		bne.s	.chactive		; if so, branch
		tst.b	toVolEnv(a1)		; check if volume envelope is enabled
		beq.s	.chinactive		; branch if not

.chactive
		moveq	#$40,d0			; set TL modulation as fully active
		or.b	#$80,(a1)		; set operator as active

.chinactive
		add.w	d2,(a1)			; go to next channel
		dbf	d1,.chloop		; loop for each channel
		or.b	d0,(a6)			; or the active bit value to first operator
		rts

dcComplexTable:
.entry		macro extra, first, second
	dc.w *-\first, *-\second
	dc.b \extra, 0, \extra, 0
    endm

    .entry	0, dcResetVolEnvTL, dcModOnTL_Normal	; %0000: Setup modulation and reset volume envelope
    .entry	0, dcModOnTL_Normal, dcComplexRts	; %0001: Setup modulation
    .entry	1, dcVolEnvTL_Normal, dcComplexRts	; %0010: Setup volume envelope
    .entry	1, dcVolEnvTL_Normal, dcModOnTL_Normal	; %0011: Setup modulation and volume envelope
    .entry	1, dcModOffTL_Normal, dcComplexRts	; %0100: Disable modulation
    .entry	0, dcModOnTL_Normal, dcComplexRts	; %0101: Enable modulation
    .entry	1, dcResetVolEnvTL, dcModOffTL_Normal	; %0110: Disable modulation and reset volume envelope
    .entry	0, dcResetVolEnvTL, dcModOnTL_Normal	; %0111: Enable modulation and reset volume envelope
    .entry	1, dcVolEnvTL_Normal, dcModOffTL_Normal	; %1000; Setup volume envelope and disable modulation
    .entry	0, dcVolEnvTL_Normal, dcModOnTL_Normal	; %1001; Setup volume envelope and enable modulation
