...
.op		= 0
		rept 4
			...
			if .op=0
				btst	#0,(a6)			; check if modulation is enabled
			else
				btst	#0,(toSize*.op)(a6)	; check if modulation is enabled
			endif
			beq.s	.open\#.op\_\@			; if not, branch

			move.l	toMod+(toSize*.op)(a6),a1	; get modulation data address
			clr.w	cModFreq+(toSize*.op)(a6)	; clear frequency offset
			move.b	(a1)+,cModSpeed+(toSize*.op)(a6); copy speed

			move.b	(a1)+,d0			; get number of steps
			lsr.b	#1,d0				; halve it
			move.b	d0,cModCount+(toSize*.op)(a6)	; save as the current number of steps

			move.b	(a1)+,cModDelay+(toSize*.op)(a6); copy delay
			move.b	(a1)+,cModStep+(toSize*.op)(a6)	; copy step offset
