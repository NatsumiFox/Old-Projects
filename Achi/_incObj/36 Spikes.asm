...
SpikesSpeed:
		...
		tst.w	d1			; checkit
		bpl.w	Spikes			; if not touching floor, branch
		add.w	d1,ObY(a0)		; add to current pos
		clr.w	ObVelY(a0)		; clear vertical speed
		move.l	#ShoeSpikeCollide,(a0)	; set new routine

Spikes:
		moveq	#0,d0
		move.b	obRoutine(a0),d0
		move.w	Spik_Index(pc,d0.w),d1
		jsr	Spik_Index(pc,d1.w)
	NEXT_OBJ
; ===========================================================================
...
Spik_Hurt:
		...
		move.l	d3,obY(a0)
		jsr	(HurtSonic).l
		addq.b	#1,featurectr.w	; increase features by 1
		cmp.b	#3,featurectr.w
		blt.s	loc_CF20
	ac	ac_feature,a6
		jsr	UpdateSRAM.w
