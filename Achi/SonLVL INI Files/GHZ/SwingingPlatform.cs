...
namespace S1ObjectDefinitions.GHZ
{
	class SwingingPlatform : ObjectDefinition
	{
		...
		{
			imgwreckingball = ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../artnem/GHZ Giant Ball.bin", CompressionType.Nemesis), "../_maps - Copy/GHZ Ball.asm", 1, 2);
			for (int i = 0; i < labels.Length; i++)
				imgs.Add(ObjectHelper.MapASMToBmp(ObjectHelper.OpenArtFile("../artnem/GHZ Swinging Platform.bin", CompressionType.Nemesis), "../_maps - Copy/Swinging Platforms (GHZ).asm", labels[i], i == 1 ? 0 : 2));
		}
